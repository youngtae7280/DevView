...
- [Natural Language Request Intake Boundary](concept/natural-language-request-intake-boundary.md) - Preview-only DevView
  ...
  deterministic validation, graph traversal planning, selected graph slices, and contract compiler input without
  implementing an AI classifier, runtime LLM calls, graph traversal, instruction-pack generation, graph-source mutation,
  apply, or approval.

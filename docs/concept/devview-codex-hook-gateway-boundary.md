...
and hook script template preview command are measured in the advisory activation-readiness lane. They read existing
preview/report artifacts and write only explicit preview outputs. They still do not install hooks, trust commands, block
Codex execution, or enforce DevView. Future hook scripts should remain lightweight and fit inside the advisory 5 second
deterministic DevView runtime budget.

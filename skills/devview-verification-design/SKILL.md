---
name: devview-verification-design
...
.devview/blueprint/verification-plan.md
.devview/control/visual-verification-profile.json
.devview/control/hardware-readiness-ledger.json
...
7. Create a root-level acceptance plan for the current slice.
   7a. When the parity/completeness profile applies, add visual/runtime parity checks for relevant surfaces and update `.devview/control/visual-verification-profile.json`.
   7b. For hardware-dependent Work nodes, record readiness verification state in `.devview/control/hardware-readiness-ledger.json`.
...
10. Save `.devview/tree/test-tree.json`.
...
Use `[Codex 硫붾え]` only for short verification rationale or risk notes.

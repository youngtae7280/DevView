---
name: devview-run-revision
description: Execute a bounded DevView revision pack against affected reopened nodes only, rerun impacted tests, refresh evidence, and resubmit for review.
---
...
`devview-run-revision` is a helper skill for executing a bounded revision after Change / Impact / Revision context exists.
...
```text
.devview/revisions/rev-*/revision-manifest.json
.devview/revisions/rev-*/06-revision-task-cards/
.devview/revisions/rev-*/07-regression-checks.md
...
Use `[Codex 硫붾え]` only for short explanation of revision risk or validation interpretation.

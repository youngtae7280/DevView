...
Abstract quality expressions include `clean`, `nice`, `fast`, `intuitive`, `stable`, `easy to use`, `modern`, `efficient`, `flexible`, `scalable`, `problem-free`, and Korean equivalents such as `源붾걫?섍쾶`, `蹂닿린 醫뗪쾶`, `鍮좊Ⅴ寃?, `?덉젙?곸쑝濡?, `?ъ슜?섍린 ?쎄쾶`, `吏곴??곸쑝濡?, `?꾨??곸쑝濡?, `?⑥쑉?곸쑝濡?, `?좎뿰?섍쾶`, `?뺤옣 媛?ν븯寃?, and `臾몄젣 ?녾쾶`.
...
"Create a technical explanation deck about DevView for software seminar attendees."
...
Use `.devview/control/decision-queue.json` for questions that matter to product meaning or execution safety. Each decision should include:
...
Read `.devview/blueprint/requirement-tree.json`.

If `.devview/tree/product-tree.json` exists, use it to confirm product status and scope before selecting the compatibility requirement node. If the two views disagree, stop and report the mismatch instead of guessing.

Select the current node in this order:

1. `devview-state.json.currentNodeId` if that node still needs work.
...
The state card must make clear whether DevView is continuing automatically or stopping at the UI/UX gate.
